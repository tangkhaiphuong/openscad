...
module box() {
  ...
  difference() {
    ...
    let(_thickness_double = thickness * 2, _width = width + _thickness_double,
      ...
      up(_thickness_double / 2) { cuboid(size = _size, rounding = fillet); }
    }

    // The inner box
    up(thickness) { cube(size = size, center = true); }
...
module lid() {
  ...
  translate(v = _v) {
    ...
    difference() {
      ...
      let(_x = width / 2 + thickness - fillet,
          _z = thickness - fillet - tolerance, _v = [ _x, 0, _z ]) {
        translate(v = _v) {
          difference() {
            let(_width = fillet * 2, _height = fillet * 2, _depth = depth * 2,
                _size = [ _width, _depth, _height ]) {
